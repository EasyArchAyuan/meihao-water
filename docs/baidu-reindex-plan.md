...
## 十一、「验证失败：无法连接到您网站的服务器」（第三轮）排查记录

> **结论：根因是我们对外宣告了 HTTP/3，但那一条路被云防火墙堵死。**
> 这是一个「**只能被客户端触发、命令行永远复现不了**」的 bug。已在服务器侧修好，
> 并用**真实 HTTP/3 请求**验证通过。
...
| 时间 | 来源 | 结果 |
|---|---|---|
| 20:37:13 | 浏览器（`Referer: https://ziyuan.baidu.com/`） | **304**（= 上一轮截图那条） |
| 20:37:20 | 浏览器 | **304** |
| 20:46:00 | 浏览器 | **200 / 32B** |
| 20:48:28 | 浏览器 | **200 / 32B + `no-store`** |
| 20:37 → 20:48 | **Baiduspider**（`112.34.110.28/.29/.156`，共 16 次） | **全部 200 / 32B** |

- 两条 304 都发生在 `(verifyfile)` 上线**之前**；其后 28 条验证请求**全部 200** → **304 确已解决。**
- 全站只被请求过**一个**验证文件名（31 次），不存在「平台换了文件名被我们 404」；
  唯一 404 是扫描器打的 `/.env_default`。
- 全站状态分布：44×200 / 15×301 / 2×304 / 2×308 / 1×403 / 1×404。

### 11.2 真根因：宣告了 HTTP/3，UDP 443 却不通

```
Caddy 响应头：Alt-Svc: h3=":443"; ma=2592000   ← 告诉客户端「我也支持 QUIC」
Caddy 监听：  udp UNCONN *:443  (caddy)        ← 确实在 UDP 443 上听
Lighthouse 防火墙：TCP 80 ✅  TCP 443 ✅  UDP 443 ❌ 没有这条规则
```

UDP 443 入向被**静默丢弃**（不是拒绝，是无任何回应）。客户端（浏览器、无头检测端）
读到 `Alt-Svc` 后改走 QUIC，就**无声无息地卡到超时** —— 报出来正是
「无法连接到您网站的服务器」。

⚠️ **为什么我们所有自测都正常**：`curl` 既**不支持 HTTP/3**，也**忽略 `Alt-Svc`**，
永远走 TCP。所以「服务端全绿、客户端连不上」这种矛盾，用 curl 测试**在原理上就发现不了**。

### 11.3 交叉验证（境外可达性）

- `check-host.net` 先后 16 个境外节点测该验证文件 → **全部 `Connection reset by peer`**。
- 服务器侧 `tcpdump`：TCP 三次握手通、对方发来 ClientHello(1412B)、我们也发出了完整的
  ~5006B ServerHello，**但对方 RST 里的 `ack=1`（说明它一个字节都没收到我们的响应）**。
  → 我方大包到不了对方、纯 ACK 能到，属境外链路丢包 / PMTU 类问题。
- 已排除本机拦截：`iptables` 的 `YJ-FIREWALL-INPUT` 只有几条**特定 IP** 的 REJECT，
  且**计数器全为 0**；`ufw` inactive；`fail2ban` inactive。

### 11.4 修法与验证（已上线）

```
① 云防火墙补一条（Lighthouse MCP: create_firewall_rules）
   Protocol=UDP  Port=443  CidrBlock=0.0.0.0/0  Action=ACCEPT
   描述：HTTP3 QUIC for meihao-shuiye site
```

验证**不用 curl**（它不支持 h3），改用 `aioquic` 自写一个最小 HTTP/3 客户端：

```
客户端：QUIC_HANDSHAKE_OK
        H3_RESULT status=200 body=b'c4a4c69f87e783677853e5536bf16d8e'
服务端：remote_ip=…  proto=HTTP/3.0  uri=/baidu_verify_…html  status=200  size=32
```

→ **HTTP/3 端到端打通**。服务端访问日志里出现 `"proto":"h3"` 即为铁证。

### 11.5 一个改变判断的域名史事实

| | IP | 归属 |
|---|---|---|
| 旧站 | `103.66.92.229` | **中国香港**（AS55933 Cloudie Limited，Tung Chung） |
| 新站 | `49.233.87.42` | 北京 · 腾讯云（AS45090） |

**香港托管不需要备案；搬到境内服务器则必须办「接入备案」。** 百度在中国大陆收录的前提是
备案，`company.icp` 仍为 `TODO`。→ 这是与验证**并行**的硬前置条件。

### 11.6 可复用的判读经验

> **遇到「自测全对、客户端报连不上」，先问一句：我们对外宣告了哪些协议，那些路是否真的通？**
> `Alt-Svc` 是最容易踩的一种：**宣告协议 ≠ 那条路可达**，而且中间可能被防火墙**静默**丢弃
> （没有 RST、没有超时提示，只有"连不上"）。
>
