...
- 🔴 **Caddy 运维三条铁律**（2026-09-22 全部实测踩过）：
  ...
  4. 🔴 **宣告了 `Alt-Svc: h3` 就必须放通 UDP 443**（2026-09-22 实测）：Caddy 默认同时听 TCP+UDP 443，响应带 `Alt-Svc: h3=":443"; ma=2592000`；但 Lighthouse 防火墙**只有 TCP 80/443** → **QUIC 入向被静默丢包**，浏览器/无头检测端按提示改走 QUIC 就**无声卡到超时**，表现正是「无法连接到您网站的服务器」。⚠️ **curl 不走 QUIC、也忽略 Alt-Svc，所以所有自测永远正常、复现不了**（这就是「我测 200、平台说连不上」的隐藏来源之一）。修法二选一：① 防火墙放通 **UDP 443**（推荐，QUIC 真能用；境外链路丢包时 QUIC 抗丢包也更好）；② Caddy 全局 `servers { protocols h1 h2 }` 关 h3（对**已缓存** Alt-Svc 的客户端无效，Chrome 记 30 天）。推论：**凡「自测全对、客户端报连不上」，先查『我们宣告了什么协议、那条路是否真通』。** ✅ 本站已于 **2026-09-22 放通 UDP 443** 并用 aioquic 实测 HTTP/3 拿到 `200` + 服务端日志 `"proto":"h3"` 通过验证；本机 curl 不支持 h3，**验证 h3 一律用 aioquic，别用 curl**（配方见技能 `baidu-site-migration` 5.3 节）。
